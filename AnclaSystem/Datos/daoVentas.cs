...
namespace Datos
{
    ...
    public class daoVentas
    {
        ...
        public List<Ventas> obtenerTodos(String FECHA, int ID_CAJERO)
        {
            List<Ventas> listaVentas = new List<Ventas>();
            Conexion cn = new Conexion();
            ...
            try
            {
                string query = "select V.ID, V.TOTAL,V.FECHA,V.ID_CAJERO,U.NOMBRE from VENTAS AS V JOIN USUARIOS AS U ON V.ID_CAJERO=U.ID WHERE V.ID_CAJERO=" + ID_CAJERO+ " AND DATE(V.FECHA) ='"+FECHA+"';";
                foreach (List<object> l in cn.ejecutarConsulta(query))
                {
                    Ventas nueva = new Ventas();
                    nueva.ID = int.Parse(l[0].ToString());
